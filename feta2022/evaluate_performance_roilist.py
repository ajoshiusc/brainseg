...
def eval_model(snapshot, roi_id):
    ...
    for subbase in lst:
        ...
        auto_tmp_filename = 'auto.tmp.label.nii.gz'
        grnd_trth_tmp_filename = 'grnd.tmp.label.nii.gz'
        ...
        grnd_trth_tmp = sitk.ReadImage(ground_truth, sitk.sitkInt16)
        simg = sitk.GetArrayFromImage(grnd_trth_tmp)  # == roi_id
        simg = np.int16(simg == roi_id)
        simgitk = sitk.GetImageFromArray(simg)
        simgitk.CopyInformation(grnd_trth_tmp)
        sitk.WriteImage(simgitk, grnd_trth_tmp_filename)

        auto_tmp = sitk.ReadImage(output_file, sitk.sitkInt16)
        simg = sitk.GetArrayFromImage(auto_tmp)  # == roi_id
        simg = np.int16(simg == roi_id)
        simgitk = sitk.GetImageFromArray(simg)
        simgitk.CopyInformation(auto_tmp)
        sitk.WriteImage(simgitk, auto_tmp_filename)

        cmd = '/home/ajoshi/webware/EvaluateSegmentation-2020.08.28-Ubuntu/EvaluateSegmentation ' + \
            grnd_trth_tmp_filename + ' ' + auto_tmp_filename + ' -xml ' + xmlfile
...
if __name__ == "__main__":
    ...
    for roi_id in [2,3,4,5,6,7]:
        ...
        epoch_mean.to_csv('epoch_mean_'+str(roi_id)+'.csv')
        epoch_var.to_csv('epoch_var_'+str(roi_id)+'.csv')
